<template>
  <div
    class="p-6 pt-[calc(60px+4rem)] pb-[4rem] lg:py-[calc(60px+2.5rem)] lg:p-10 lg:pr-16 flex flex-col justify-start items-center"
  >
    <div class="flex justify-center flex-col items-start mb-4 w-full">
      <h1
        class="text-[2rem] font-bold pl-4 pb-4 border-l-[3px] border-b-[3px] border-sky-400 text-slate-200"
      >
        Welcome to Cardricks!
      </h1>
      <h2 class="text-[1.2rem] my-4 text-center">
        At Cardricks, you can learn card tricks and perform their right inside
        your browser.
      </h2>
      <p class="text-[1rem] text-justify my-4">
        Dive into the learning of some cards magic tricks performed by well
        known magicians all over the world. These tricks are compiled from all
        over the internet. There are playgrounds for each trick where you can
        perform the corresponding trick by following the mentioned process. The
        tricks menu is shown on the left side of the webpage. You can click on
        any trick name in order to expand or shrink it. You can click on any
        sublink inside a trick in order to jump to that.
      </p>
      <p class="text-[1rem] my-4">Happy Learning!</p>
    </div>
    <div class="flex justify-center flex-col items-center mb-8 w-full">
      <p class="font-bold text-[1.7rem] my-4 text-slate-200">
        Learn The Tricks!
      </p>
      <button
        type="submit"
        class="bg-slate-700 hover:text-sky-400 hover:bg-slate-800 text-slate-200 font-bold py-1 px-4 rounded text-[0.8rem]"
        @click="startTricks"
      >
        Start
      </button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { useRouter } from "vue-router";
import { onMounted } from "vue";

const router = useRouter();

const startTricks = () => {
  router.push({ path: "/tricks/basic-tricks/1" });
};

onMounted(() => {
  scrollToElement();
});

function scrollToElement() {
  const element = document.getElementById("mainApp");
  if (element) {
    element.scrollIntoView();
    window.scrollTo(0, 0);
  }
}
</script>

<style scoped>
h1,
h2,
p {
  cursor: text;
}
</style>
